...
from traj_follow_policy import TrajFollowPolicy
...
def train(args):
    ...
    if args.env == 'MetaDriveInfoEnv':
        ...
        rtg_target = 2000
        env_d4rl_name = f'expert_dataset_traj'
        drive_test_config = dict(
            ...
            accident_prob=0.0,
            horizon=1000,
            agent_policy=TrajFollowPolicy,

        )
        env = SafeMetaDriveEnv(drive_test_config)
        _, _ = env.reset()
        drawer = env.engine.make_point_drawer(env.vehicle.origin, scale=1)
        drawer.setH(90)
    else:
    ...
    for i_train_iter in range(max_train_iters):
        ...
        results = evaluate_on_env(model, device, context_len, env, rtg_target, rtg_scale,
                                  num_eval_ep, max_eval_ep_len, state_mean, state_std,render=False,drawer=drawer)
